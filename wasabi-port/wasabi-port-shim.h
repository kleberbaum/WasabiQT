...
// reference but the leaked source never defined for non-Win32/macOS.
//
// These are declarations only. The Maki VM and script registry, // the only parts of Wasabi we actually compile, never CALL the file
...
//
// The leaked replicant/foundation/types.h has Linux + x86_64 only.
...
// is a macro so it doesn't need GUID to be defined at this point, // the dispatched types.h header (linux-amd64/types.h) defines GUID
...
// the leaked vcpu.cpp doesn't, but consumers of the shim might.
